...
function App() {
  ...
  const handleAddProductLine = (product) => {
    setQuoteLines((prev) => {
      const existingIndex = prev.findIndex((l) => l.product_id === product.id);
      if (existingIndex !== -1) {
        const clone = [...prev];
  ...
  const handleCreateQuote = async (e) => {
    ...
    try {
      ...
      setQuoteSuccess(
        `Cotización creada correctamente: ${res.name || ''} (ID ${res.order_id})`
      );
      resetQuoteForm();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="app">
      ...
      <nav className="app-tabs" style={{ marginBottom: '1rem' }}>
        <button
          type="button"
          className={activeTab === 'deliveries' ? 'tab-active' : ''}
          onClick={() => setActiveTab('deliveries')}
        >
          Entregas vía distribuidor
        </button>
        <button
          type="button"
